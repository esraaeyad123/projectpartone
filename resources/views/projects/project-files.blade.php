...
@section('content')
            ...
            Array.from(e.target.files).forEach(file => {
                ...
                .then(uploadedFile => {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'نجاح',
                        text: `✔️ تم رفع ملف "${uploadedFile.name}" بنجاح`
                    });
                })
                .catch(err => {
                    console.error(err);
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.getElementById('project-name-display').textContent =
            `(${currentProjectName} - ID: ${currentProjectId})`;
    ...
    function viewFile(fileId) {
        window.open(`/projects/files/${fileId}/view`, '_blank');
    }
    ...
    function openUploadModal() {
        ...
        if (uploadModal) {
            ...
            if (uploadModalProjectNameElem) uploadModalProjectNameElem.textContent = currentProjectName;
        }
    }
    ...
    // =================== تنزيل الملفات المحددة بشكل منفصل ===================
    function downloadSelectedFiles() {
        ...
        // A function that starts the download of a single file
        const downloadFileSequentially = (index) => {
            if (index < selected.length) {
                const fileId = selected[index];
                const link = document.createElement('a');
                link.href = `/projects/files/${fileId}/download`;
                link.download = ''; // This attribute forces a download dialog
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);

                // Wait a little before downloading the next file
                setTimeout(() => {
                    downloadFileSequentially(index + 1);
        ...
        // Start the download process for the first file
        downloadFileSequentially(0);
    }
    ...
    async function downloadSelectedFilesAsZip() {
        ...
        for (const fileId of selected) {
            try {
                const response = await fetch(`/projects/files/${fileId}/download`);
        ...
        .then(content => {
            const projectName = currentProjectName.replace(/[^a-z0-9]/gi, '_').toLowerCase();
            const filename = `${projectName}_files.zip`;
    ...
    // DropZone يستدعي الدالة الموحدة
    function setupDropZone() {
        const dropZone = document.getElementById('dropZone');
        const fileInput = document.getElementById('fileInput');

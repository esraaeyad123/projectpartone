@extends('layouts.app')
@section('title', __('LIMS - Employee Files'))
@section('content')
...
<main class="main-content">
    <section id="employee-files-section" class="section-content active">
        <h2>
            <i class="fas fa-folder"></i>
            <span data-key="Employee Files">Employee Files</span>
            <span id="employee-name-display" class="employee-name-display"></span>
        </h2>
        ...
        <div class="icon-toolbar">
            <div>
                ...
                <button title="Delete Selected" onclick="deleteSelectedEmployeeFiles()" class="btn-icon">
                    <i class="fas fa-trash"></i>
                </button>
                <button title="Download Selected as Zip" onclick="downloadSelectedEmployeeFilesAsZip()" class="btn-icon">
                    <i class="fas fa-file-archive"></i>
                </button>
                <button title="Download Selected" onclick="downloadSelectedEmployeeFiles()" class="btn-icon">
...
<div id="uploadFileModal" class="modal">
    <div class="modal-content">
        ...
        <h3><span data-key="Upload File for">Upload File for</span> <span id="uploadModalEmployeeName"></span></h3>
        <form id="fileUploadForm" onsubmit="uploadEmployeeFile(event)">
...
const currentEmployeeName = "{{ $employee->full_name }}";
const csrfToken = '{{ csrf_token() }}';
const fileInput = document.getElementById('fileInput');

// =================== عند تحميل الصفحة ===================
document.addEventListener('DOMContentLoaded', () => {
    const employeeDisplay = document.getElementById('employee-name-display');
    if(employeeDisplay) employeeDisplay.textContent = `(${currentEmployeeName} - ID: ${currentEmployeeId})`;

    loadAndRenderEmployeeFiles();
    setupSearchFilter();
    setupDropZone();
});
    ...
    fileCard.innerHTML = `
        ...
        <div class="file-card-hover-details">
            ...
            <div class="file-card-actions-hover">
                <button class="btn-icon view-file-btn" title="View" onclick="viewEmployeeFile(${file.id})">
                    <i class="fas fa-eye"></i>
                </button>
                <button class="btn-icon download-file-btn" title="Download" onclick="downloadEmployeeFile(${file.id})">
                    <i class="fas fa-download"></i>
                </button>
                <button class="btn-icon delete-file-btn" title="Delete" onclick="deleteEmployeeFile(${file.id})">
                    <i class="fas fa-trash"></i>
                </button>
            </div>
        </div>
...
// =================== Load & Upload ===================
function loadAndRenderEmployeeFiles() {
    const container = document.getElementById('fileIconsContainer');
    if(!container) return;

    container.innerHTML = '<p class="no-files-message">Loading files...</p>';

    fetch(`/employees/${currentEmployeeId}/files-json`)
        .then(res => res.json())
        .then(files => {
            container.innerHTML = '';
            if(!files.length){
...
function processAndUploadEmployeeFiles(files) {
    ...
    Array.from(files).forEach(file => {
        ...
        fetch(`/employees/${currentEmployeeId}/files`, { method: 'POST', body: formData })
        .then(res => res.json())
        .then(uploadedFile => {
...
function uploadEmployeeFile(event){
    ...
    const files = document.getElementById('fileInputModal').files;
    processAndUploadEmployeeFiles(files);
...
function viewEmployeeFile(fileId){ window.open(`/employees/files/${fileId}/view`,'_blank'); }
function downloadEmployeeFile(fileId){ window.location.href=`/employees/files/${fileId}/download`; }
...
function deleteSelectedEmployeeFiles(){
...
async function downloadSelectedEmployeeFilesAsZip(){
    const selected = Array.from(document.querySelectorAll('.file-card-checkbox:checked')).map(cb=>cb.value);
    ...
    for(const fileId of selected){
        try{
            const response = await fetch(`/employees/files/${fileId}/download`);
    ...
    zip.generateAsync({type:"blob"}).then(content=>{
        saveAs(content,`${currentEmployeeName.replace(/[^a-z0-9]/gi,'_').toLowerCase()}_files.zip`);
...
function setupDropZone() {
    ...
    dropZone.addEventListener('drop', e => {
        ...
        if(files.length) processAndUploadEmployeeFiles(files);
    });

    dropZone.addEventListener('click', () => fileInput.click());

    // Sync hidden input with modal input
    fileInput.addEventListener('change', e => {
        if(e.target.files.length) processAndUploadEmployeeFiles(e.target.files);
    });
}
...
function openUploadModal(){
    ...
    if(modal){
        ...
        if(form) form.reset();
        const nameElem = document.getElementById('uploadModalEmployeeName');
        if(nameElem) nameElem.textContent = currentEmployeeName;
    }
}
...
function viewEmployeeFile(fileId) {
    window.open(`/employees/files/${fileId}/view`, '_blank');
}

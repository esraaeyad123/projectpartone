...
@section('content')
    ...
    if (fileInput) {
        fileInput.addEventListener('change', e => {
            // قم بحذف كل شيء داخل هذا المستمع
    ...
    function downloadSelectedFiles() {
        ...
        const downloadFileSequentially = (index) => {
            if (index < selected.length) {
                ...
                link.href = `/customer-files/${fileId}/download`;
    ...
    async function downloadSelectedFilesAsZip() {
        ...
        .then(content => {
            const customerName = currentCustomerName.replace(/[^a-z0-9]/gi, '_').toLowerCase();
            const filename = `${customerName}_files.zip`;
